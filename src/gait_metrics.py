...
def histogram_features(column_data, global_ranges, bins=5, feature_name=None):
    ...
    # Automatic range fallback if not in global_ranges (makes it more robust/less hardcoded)
    ...
    # Normalize to probabilities (proportions) for shape-focused features
    total = hist.sum()
...
def calculate_statistics(result, window_sec, window_len, analyze_bouts):
    ...
    # Ensure window_days is integer and 1D
    ...
    # Validate inputs for np.bincount
    if len(window_days) > 0 and len(window_days) == len(pred_window_steps):
        if np.all(window_days >= 0): # Ensure non-negative indices
...
def main():
    ...
    args = argparse.Namespace(
        ...
        filepath=r"N:\Gait-Neurodynamics by Names\Yonatan\gait-metrics\rush-sample.cwa.gz",
        outdir="outputs/",
        model_path=None,
    ...
    gait_detection_model = joblib.load('models/gait_detection_model_mobD_cap24.joblib').to(device)
    gait_detection_model.eval()
    step_count_model = joblib.load('models/step_count_model.joblib').to(device)
    step_count_model.eval()
    gait_speed_model = joblib.load('models/gait_speed_model.joblib').to(device)
    gait_speed_model.eval()
    cadence_model = joblib.load('models/cadence_model.joblib').to(device)
    cadence_model.eval()
    stride_length_model = joblib.load('models/stride_length_model.joblib').to(device)
    stride_length_model.eval()
    regularity_model = joblib.load('models/regularity_model.joblib').to(device)
    regularity_model.eval()
    ...
    if analyze_bouts:
        ...
        bout_starts = np.minimum(bout_starts, len(days_array) - 1)  # Prevent OOB
        bout_days = days_array[bout_starts]
    else:
        bouts_id_windows = np.array([])
        bout_days = np.array([])

    # Walking windows mask
    walk_mask = pred_walk == 1  # Assuming 1 == walking
    ...
    del acc_win_all
    # Vectorized window_days: day of first sample in each walking window
    window_starts = walking_window_indices * window_len
    window_starts = np.minimum(window_starts, len(days_array) - 1)  # Prevent OOB
